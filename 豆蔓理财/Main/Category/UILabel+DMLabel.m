...
@implementation DMAttributeModel
...
- (NSMutableArray *)attributeSrings {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setAttributeStrings:(NSMutableArray *)attributeStrings {
    objc_setAssociatedObject(self, @selector(attributeSrings), attributeStrings, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (id<DMAttributeTapActionDelegate>)delegate {
    return objc_getAssociatedObject(self, _cmd);
}
- (void)setDelegate:(id<DMAttributeTapActionDelegate>)delegate {
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

//点击方法
- (void)DM_addAttributeTapActionWithStrings:(NSArray<NSString *> *)strings tapClicked:(void (^)(NSString *, NSRange, NSInteger))tapClick {
    
    [self DM_getRangesWithStrings:strings];
    if (self.tapBlock != tapClick) {
        self.tapBlock = tapClick;
    }
}

- (void)DM_addAttributeTapActionWithStrings:(NSArray<NSString *> *)strings delegate:(id<DMAttributeTapActionDelegate>)delegate {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self DM_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(DM_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate DM_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self DM_saveEffectDicWithRange:range];
            
            [self DM_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self DM_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)DM_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self DM_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self DM_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            NSInteger link_count = self.attributeSrings.count;
            
            for (int j = 0; j < link_count; j++) {
                
                DMAttributeModel *model = self.attributeSrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(DM_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)DM_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)DM_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)DM_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)DM_saveEffectDicWithRange:(NSRange)range
{
...
- (void)DM_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf DM_getStringWithRange:range]];
            
            DMAttributeModel *model = [DMAttributeModel new];
            
            model.range = range;
            
            model.str = obj;
            
            [weakSelf.attributeSrings addObject:model];
...
- (NSString *)DM_getStringWithRange:(NSRange)range
{
